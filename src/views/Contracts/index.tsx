...
import { FileDown, FileUp, Share2, Printer, AlertCircle, BadgeCheck, AlertTriangle, FileText, Ship } from 'lucide-react';
...
const defaultColumns: Column[] = [
  ...
  { key: 'loadingStartDate', header: 'LOADING', width: '120px' },
...
export default function ContractsPage() {
  ...
  const { 
    ...
    isImporting,
    progress
  } = useImport('contracts', async (data) => {
  ...
  const handleReviewPendingClick = () => {
    ...
    const missingDocsIds = contractsWithMissingDocs.map(contract => contract.id);
    if (missingDocsIds.length > 0) {
      setSearchQuery('');  // Clear any existing search
      const missingDocsFilter = createFilterGroup('missingDocs', 'Missing Documents', missingDocsIds);
  ...
  const filterGroups = [
    ...
    createFilterGroup('productType', 'Product Type', Array.from(new Set(contracts.map(c => c.productType)))),
    createFilterGroup('originCountry', 'Origin Country', Array.from(new Set(contracts.map(c => c.originCountry)))),
    createFilterGroup('deliveryCountry', 'Delivery Country', Array.from(new Set(contracts.map(c => c.deliveryCountry)))),
    createFilterGroup('missingDocs', 'Missing Documents', [])
  ];

  const handleRowClick = (row: any) => {
    // Get buyer and seller details including ISCC info
    const buyer = row.buyerId ? partyStore.getById('buyers', row.buyerId) : null;
    const seller = row.sellerId ? partyStore.getById('suppliers', row.sellerId) : null;
        ...
        sections={[
                ...
                value: (
                  <div className="space-y-1">
                    ...
                    <div className={`flex items-center gap-1.5 text-xs ${
                      selectedRow?.buyerIscc ? 'text-green-600' : 'text-amber-600'
                    }`}>
                      {selectedRow?.buyerIscc ? (
                        <>
                          ...
                          <div>
                            ...
                            <div>Expires: {new Date(selectedRow.buyerIsccExpiry).toLocaleDateString()}</div>
                          </div>
                        </>
                      ) : (
                        <>
              ...
              { 
                ...
                value: (
                  <div className="space-y-1">
                    ...
                    <div className={`flex items-center gap-1.5 text-xs ${
                      selectedRow?.sellerIscc ? 'text-green-600' : 'text-amber-600'
                    }`}>
                      {selectedRow?.sellerIscc ? (
                        <>
                          ...
                          <div>
                            ...
                            <div>Expires: {new Date(selectedRow.sellerIsccExpiry).toLocaleDateString()}</div>
                          </div>
                        </>
                      ) : (
                        <>
          ...
          }] : []),
          {
            ...
            rows: [
              ...
              { label: 'Allowed Variance', value: `${selectedRow?.allowedVariance}%` },
              { label: 'Unit Price', value: `${selectedRow?.currency} ${selectedRow?.unitPrice}` },
              { label: 'Payment Terms', value: selectedRow?.paymentTerms },
              { label: 'Packing Standard', value: selectedRow?.packingStandard },
            ],
          },
          {
          ...
          },
          {
            ...
            rows: [
              ...
              { label: 'Delivery Country', value: selectedRow?.deliveryCountry },
              { 
                label: 'Delivery Port', 
              ...
              { label: 'Loading Start Date', value: new Date(selectedRow?.loadingStartDate).toLocaleDateString() },
              { label: 'Delivery Date', value: new Date(selectedRow?.deliveryDate).toLocaleDateString() },
            ],
          },
        ]}
        relatedItems={[
          {
            ...
            items: (selectedRow?.shipments || []).map(shipment => ({
              id: shipment.id,
              title: shipment.id,
