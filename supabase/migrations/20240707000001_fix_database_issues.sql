  ...
  IF NOT EXISTS (SELECT 1 FROM public.tanks LIMIT 1) THEN
    ...
    BEGIN
      ...
      FOR shipment_rec IN SELECT id, status, departure_date, arrival_date FROM shipments LOOP
        ...
        VALUES
          (gen_random_uuid(), shipment_rec.id, shipment_rec.status, 500, '{"FFA": 3.5, "IV": 53, "S": 0.15, "MI": 0.2}'::jsonb, shipment_rec.departure_date, shipment_rec.arrival_date, NOW(), NOW()),
          (gen_random_uuid(), shipment_rec.id, shipment_rec.status, 500, '{"FFA": 3.2, "IV": 52, "S": 0.12, "MI": 0.18}'::jsonb, shipment_rec.departure_date, shipment_rec.arrival_date, NOW(), NOW());
      END LOOP;
    END $inner$;
  END IF;
END $$;